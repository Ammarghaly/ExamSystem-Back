...
export const startExam = async (req, res, next) => {
  ...
  const isCreator =
    exam.teacherID && exam.teacherID.toString() === studentID.toString();
  ...
  if (!isEnrolled) {
    return next(
      new Error("You are not enrolled in this exam's group", { cause: 403 }),
    );
  }
  ...
  if (isCreator) {
    ...
    const activeAttempt = await ExamAttemptModel.findOne({
      examID: examId,
      studentID,
      endTime: { $exists: false },
    });
      ...
      return successResponse({
        ...
        data: {
          ...
          exam: {
            ...
            subject: Array.isArray(exam.groupID)
              ? exam.groupID[0]?.subject
              : exam.groupID?.subject,
          },
          questions,
        },
      });
  ...
  if (!isCreator) {
    const finishedAttempt = await ExamAttemptModel.findOne({
      examID: examId,
      studentID,
      endTime: { $exists: true },
    });
    if (finishedAttempt) {
      return next(
        new Error("You have already submitted this exam", { cause: 409 }),
      );
    }
  }
  ...
  if (!isCreator && exam.teacherID) {
    ...
    if (teacher && teacher.role === "Teacher") {
      ...
      await teacher.save();
      console.log(
        `💸 Deducted 0.5 credits from teacher ${teacher.name} for student exam entry. New balance: ${teacher.available_credits}`,
      );
    }
  }
...
export const getAttemptResult = async (req, res, next) => {
  ...
  const attempt = await ExamAttemptModel.findById(attemptId)
    .populate({
      ...
      select:
        "title options correctAnswer typeQue difficulty cognitiveLevel ai_explanation",
    })
    .populate({
      path: "examID",
      select:
        "title durationMinutes groupID teacherID createdAt allowReview numOfQuestion",
      populate: {
        path: "groupID",
  ...
  const totalQuestions =
    attempt.examID?.numOfQuestion || attempt.answers.length;
  const correctCount = attempt.answers.filter((a) => a.isCorrect).length;
  const incorrectCount = totalQuestions - correctCount;
  const percentage =
    totalQuestions > 0
      ? Math.round((attempt.totalScore / totalQuestions) * 100)
      : 0;

  const isPractice =
    attempt.examID?.teacherID?.toString() === studentID.toString();
  ...
  const formattedAnswers = attempt.answers.map((a, index) => {
    ...
    if (allowReview) {
      ...
      questionObj.ai_explanation = a.isCorrect
        ? null
        : a.questionId?.ai_explanation;
    }
